    ...
    realTimeDb.ref().child('users').orderByChild('fullname').startAt(name).endAt(name + "\uf88f").on("value", function (snapshot) {
        const values = snapshot.val();
            if (values && values.length !== 0) {
                ...
                const users = keys.map(key => value[key]);
                filterUsers(authentcator, users);
            } else {
                setUsers(() => []);
            }
    });
...
const updateUsers = async (authenticatedUser, selectedUser) => {
    ...
    setIsLoading(true);
    const atuhenticatedUserPending = { id: authenticatedUser.id, fullname: authenticatedUser.fullname, email: authenticatedUser.email, avatar: authenticatedUser.avatar };
    const selectedUserPending = { id: authenticatedUser.id, fullname: selectedUser.fullnae, email: selectedUser.email, avatar: selectedUser.avatar };
